...
wm.Weltmeister = ig.Class.extend({
  ...
  resourceMap: {},
    ...
    const { name: levelName, entities } = context
    const level = wm.levels[levelName];
    const levelData = level.data();

    this.resourceMap = resourceMap;
		this.filePath = `${level.path}.data.js`;
    ...
    const { _wmui } = levelData
    if (_wmui !== undefined) {
      this.screen.x = _wmui.x;
      this.screen.y = _wmui.y;
      this.scroll(0, 0)

      this.zoomLevel = wm.zoomLevel = _wmui.zoomLevel;
      wm.config.labels.step = Math.round( this.labelsStep / this.zoomLevel );
      $('#zoomIndicator').text( this.zoomLevel + 'x' );
