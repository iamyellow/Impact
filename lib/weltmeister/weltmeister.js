...
wm.Weltmeister = ig.Class.extend({
	...
	init: function() {
		...
		this.tilesetSelectDialog = new wm.SelectFileDropdown( '#layerTileset', 'images' );
	...
	loadLevel: function( context ) {
		// from react
		const { data, name: levelName, resources, entityModules } = context
    console.log('***', context)

		this.filePath = `${wm.levels[levelName].path}.data.js`;
		this.fileName = levelName;
		this.levelData = data;
		this.entities.setEntities( entityModules );
		this.tilesetSelectDialog.setData( resources )
