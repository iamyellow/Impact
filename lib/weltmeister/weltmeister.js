...
wm.Weltmeister = ig.Class.extend({	
	...
	storageLastLevelKey: 'wmLastLevel',
	...
	init: function() {
		...
		if( wm.config.loadLastLevel ) {
			var path = localStorage.getItem( this.las);
			if( path ) {
				this.load( null, path );
			}
		}
	...
	loadNew: function() {
		localStorage.removeItem( this.storageLastLevelKey );
	...
	load: function( dialog, path ) {
		...
		var req = $.ajax({
			...
			error: function() { localStorage.removeItem( this.storageLastLevelKey ); }
		});
	},
	
	
	loadResponse: function( data ) {
		localStorage.setItem( this.storageLastLevelKey, this.filePath );
