...
wm.EditMap = ig.BackgroundMap.extend({
  ...
  tilesetName: "",
	...
	init: function( name, tilesize, tilesetSrc, tilesetName, foreground ) {
		this.name = name;
		this.parent( tilesize, [[0]], tilesetSrc || '' );
  ...
  setTilesetName: function( name ) {
	...
	draw: function() {
		...
		// when zoomed out
		if( this.visible && !(wm.zoomLevel < 1 && this.repeat) ) {
			this.drawTiled();
		}
